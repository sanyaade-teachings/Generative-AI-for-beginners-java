...
**Zaman Taahhüdü**: Tüm atölye çevrimiçi ve yerel kurulum olmadan tamamlanabilir. Ortam kurulumu 2 dakika sürer, örnekleri keşfetmek ise keşif derinliğine bağlı olarak 1-3 saat arası zaman alır.

> **Hızlı Başlangıç**

1. Bu depo’yu GitHub hesabınıza çatallayın
2. **Code** → **Codespaces** sekmesine tıklayın → **...** → **New with options...**’a tıklayın
3. Varsayılanları kullanın – bu kurs için oluşturulan Geliştirme konteyneri seçilecektir
...
- **Temel Kavramlar**: Büyük Dil Modelleri, tokenlar, yerleştirmeler ve yapay zeka yeteneklerini anlama
...
- **Azure AI Foundry**: Model dağıtımlarını kod olarak Bicep ve Azure Developer CLI (azd) ile sağlayın
- **Spring Boot + Spring AI**: Kurumsal AI uygulama geliştirme için en iyi uygulamalar
- **Anahtarsız Kimlik Doğrulama**: Microsoft Entra ID ile güvenli bağlantı - yönetilecek API anahtarı yok
...
- **Yerleştirmeler & Vektör İşlemleri**: Semantik arama ve benzerlik eşleştirme uygulamaları
- **Retrieval-Augmented Generation (RAG)**: AI’yi kendi veri kaynaklarınızla birleştirme
- **Fonksiyon Çağrıları**: AI yeteneklerini özel araçlar ve eklentilerle genişletme
- **[→ Bölüm 3’e Başla](./03-CoreGenerativeAITechniques/README.md)**

### **Bölüm 4: Pratik Uygulamalar ve Projeler**
- **Evcil Hayvan Hikayesi Üreteci** (`petstory/`): Azure AI Foundry ile yaratıcı içerik üretimi
...
- **Azure AI Foundry İçerik Güvenliği**: Yerleşik içerik filtreleme ve güvenlik mekanizmalarını test edin (sert engellemeler ve yumuşak reddetmeler)
- **Sorumlu AI Demo**: Modern AI güvenlik sistemlerinin pratikte nasıl çalıştığını gösteren örnek
- **En İyi Uygulamalar**: Etik AI geliştirme ve dağıtım için temel rehberlik
...
Yapay zeka uygulamaları geliştirirken takılırsanız veya herhangi bir sorunuz olursa, diğer öğrenenler ve deneyimli geliştiricilerle MCP hakkında tartışmalara katılabilirsiniz. Soruların memnuniyetle karşılandığı ve bilginin özgürce paylaşıldığı destekleyici bir topluluktur.
